...
export default function Home() {
  ...
  async function runConfiguredAgent(agent: AgentDefinition, prompt: string) {
    ...
    async function executeAgentConnector(action: any) {
      ...
      async function orchestratePostPush(pushArgs: Record<string, unknown>, pushData: unknown) {
        ...
        const body = String(pushArgs.body || `Automated changes from KChat agent **${agent.name}**.\n\nBranch: \`${head}\`\n\nThe agent pushed the branch from its E2B workspace. Review CI and the Netlify preview before merging.`);
        const pr = await executeAgentConnector({ tool: "github_create_pull_request", args: { owner, repo, head, base, title, body, draft: pushArgs.draft === true } });
        const prNumber = Number(pr?.number || 0);
        const prHeadSha = String(pr?.head?.sha || "");
        transcript.push(`workflow: PR #${prNumber || "?"} created for ${head} → ${base}`);

        let latestPr = pr;
        let checks: any = null;
        for (let poll = 0; poll < 3 && prNumber; poll++) {
          latestPr = await executeAgentConnector({ tool: "github_get_pull_request", args: { owner, repo, pull_number: prNumber } });
          const sha = String(latestPr?.head?.sha || prHeadSha);
          if (sha) checks = await executeAgentConnector({ tool: "github_list_checks", args: { owner, repo, ref: sha } });
          const runs = Array.isArray(checks?.check_runs) ? checks.check_runs : [];
          const current = runs.filter((run: any) => !sha || run?.head_sha === sha);
          const pending = current.filter((run: any) => ["queued", "in_progress", "waiting", "requested", "pending"].includes(String(run?.status || "").toLowerCase()));
          transcript.push(`workflow: CI poll ${poll + 1}/3 — ${current.length} checks, ${pending.length} pending`);
        ...
        if (siteId && agent.tools.includes("Netlify")) {
          const deploys = await executeAgentConnector({ tool: "netlify_list_deploys", args: { siteId } });
          const list = Array.isArray(deploys) ? deploys : (Array.isArray(deploys?.deploys) ? deploys.deploys : []);
          const candidates = list.filter((deploy: any) => {
            ...
            return (ref && prHeadSha && ref === prHeadSha) || branch === head || context === "deploy-preview";
          });
          const preview = candidates.find((deploy: any) => String(deploy?.context || "") === "deploy-preview") || candidates[0];
          if (preview) {
            const deployId = String(preview?.id || preview?.deploy_id || "");
            const detail = deployId ? await executeAgentConnector({ tool: "netlify_get_deploy", args: { siteId, deployId } }) : preview;
            const url = String(detail?.ssl_url || detail?.deploy_ssl_url || detail?.url || preview?.ssl_url || preview?.deploy_ssl_url || preview?.url || "");
            transcript.push(`workflow: Netlify preview${url ? ` — ${url}` : " found"}`);
          } else {
            transcript.push("workflow: no matching Netlify Deploy Preview found yet");
          }
        }
      ...
      if (tool.startsWith("browser_")) {
        ...
        const response = await fetch("/api/runtime", { method: "POST", signal: controller.signal, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: tool, sandboxId: sandbox, ...args }) });
        const data = await response.json().catch(() => ({ error: "Invalid browser response." }));
        if (!response.ok) throw new Error(data?.error || `${tool} failed (${response.status})`);
        transcript.push(`${tool}:\n${JSON.stringify(data).slice(0, 16000)}`);
        return data;
      }
